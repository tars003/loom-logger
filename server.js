...
client.on('message', async (topic, payload) => {
    ...
    console.log('Received Message:', topic, JSON.parse(inStr).stationId);
    ...
    // if (topic == 'aa') {
        ...
        // THERE EXISTS A ZONE IN currentZone | meaning a zone object for it has already been created 
            ...
            if(stationNames.includes(stationId)) {
                ...
                console.log(`Station already in list ${stationId} | ${stationNames.includes(stationId)}`);
                console.log();
            ...
            // STATION DOES NOT HAVE PREVIOUS DATA | meaning station object does not exist till now in the stations array
                ...
                console.log(`Station NEW insterted, ${stationId} | ${stationNames.includes(stationId)}`);
...
app.get('/getStatus', async (req, res) => {
    try {
        ...
        const zone = await Zone.findOne({ name: currZone.name });
        let result = `<h1>Online Stations ${zone.stations.length}</h1><br>`;



        zone.stations.map((station) => {
            let lastTime = station.data[station.data.length-1]['_id'];
            let a =  moment(lastTime).utc().format('DD-MM-YY HH:mm:ss');
            let now = moment().format('DD-MM-YY HH:mm:ss');
            let diff = moment(now, 'DD-MM-YY HH:mm:ss').diff(moment(a, 'DD-MM-YY HH:mm:ss'), "minutes");
            if(diff > 2) {
                result += `<p style="color: red;"><b>${station.stationName}</b> Last connected at : ${station.data[station.data.length-1]['_id']}</p>`
            } else {
                result += `<p style="color: green;"><b>${station.stationName}</b> Last connected at : ${station.data[station.data.length-1]['_id']}</p>`;
            }
